...
def construct_knn_graph(X, kNN, dist, batch_size=2000, device="cpu"):
    batch_size = int(
        batch_size * 16 / (psutil.virtual_memory().total / 2**30)
    )  # ram mem = psutil.virtual_memory().total/2**30
    ...
    if dist == "euclidean":
        ...
        if (n // batch_size) * batch_size < n:
            num_batch += 1
        print(
            "n, batch_size, num_batch, kNN, device:",
            n,
            batch_size,
            num_batch,
            kNN,
        ...
        for idx in range(num_batch):
            ...
            term2 = (
                X2[idx_current : idx_current + batch_size_current]
                .unsqueeze(1)
                .repeat(1, n)
            )  # size=[batch_size, n]
            ...
            sorted, indices = torch.sort(
                D, dim=1
            )  # from smallest to largest values, size=[batch_size, n]
    ...
    if dist == "euclidean_zelnik_perona":
        ...
        sigma = D[:, -1]
        sigma_i = sigma[
            torch.arange(n).repeat_interleave(kNN)
        ]  # sigma_i in Zelnik-Perona technique
    ...
    if dist == "cosine":
        ...
        X = X - X.mean(dim=0).unsqueeze(dim=0)  # centered data, size=[n, d]
        X = X / (X**2).sum(dim=1).sqrt().unsqueeze(
            1
        )  # normalized data / project data on unit sphere, size=[n, d]
        n = X.size(0)
        num_batch = n // batch_size
        if (n // batch_size) * batch_size < n:
            num_batch += 1
        ...
        for idx in range(num_batch):
            ...
            sorted, indices = torch.sort(
                D, dim=1, descending=True
            )  # from *largest* to smallest values, size=[batch_size, n]
...
def compute_pca(X, nb_pca):
    ...
    if n > d:
        ...
        if nb_pca < dCovX:
            lamb, U = scipy.sparse.linalg.eigsh(
                CovX, k=nb_pca, which="LM"
            )  # U = d x nb_pca
...
def compute_kernel_kmeans_EM(nc, Ker, Theta, nb_trials):
    ...
    for k in range(nb_trials):
        ...
        while (k < 30) & (diffC > 1e-2):
            ...
            O = np.diag(np.array(1.0 / (Ones.dot(Theta).dot(F) + 1e-6)).squeeze())
            T = Ker.dot(Theta.dot(F.dot(O)))
            D = -2 * T + np.repeat(
                np.diag(O.dot((F.T).dot(Theta.dot(T))))[None, :], n, axis=0
            )
...
def construct_kernel(X, type_kernel, parameters=None):
    ...
    if type_kernel == "polynomial":
        ...
        Xzc = X - np.mean(X, axis=0)  # zero-centered data
        Xl2proj = (
            Xzc.T / np.sqrt(np.sum(Xzc**2, axis=1) + 1e-10)
        ).T  # Projection on the sphere, i.e. ||x_i||_2 = 1
...
def compute_SVM(
    ...
    if type_svm == "gaussian_kernel":
        if len(param_svm) < 2:
            Ker = construct_kernel(Xtrain, "gaussian")  # TODO
        sigma = param_svm[1]
    ...
    if type_svm == "gaussian_kernel":
        Ddist = sklearn.metrics.pairwise.pairwise_distances(
            Xtrain, Xtest, metric="euclidean", n_jobs=1
        )
        sigma2 = sigma**2
    ...
    # Time steps
    sigma = 1.0 / np.linalg.norm(L, 2)
    tau = 1.0 / np.linalg.norm(Q, 2)
    ...
    while (diffx > 1e-3) & (k < 1000):
        ...
        if not (k % 50):
            ...
            # b offset
            idx_unlabeled_data = np.where(np.abs(l) < 1.0 / 2)
            alpha_labels = alpha
            alpha_labels[idx_unlabeled_data] = 0
            idx = np.where(np.abs(alpha_labels) > 0.25 * np.max(np.abs(alpha_labels)))
            Isv = np.zeros([n])
            Isv[idx] = 1  # Indicator function of Support Vectors
            ...
            C_test = np.array(
                1.0 / 2 * (1 + np.sign(f_test)), dtype=np.int64
            )  # decision function in {0,1}
            ...
            if not (plot == None):
                plt.subplot(121)
                plt.scatter(
                    Xtest[:, 0], Xtest[:, 1], s=size_vertex_plot * np.ones(n), c=f_test
                )
                plt.title("Continuous decision function")
                plt.subplot(122)
                plt.scatter(
                    Xtest[:, 0], Xtest[:, 1], s=size_vertex_plot * np.ones(n), c=C_test
                )
                plt.title(
                    "Decision function, iter="
                    + str(k)
                    + ", acc="
                    + str(round(acc_test, 3))
                )
                plt.tight_layout()
                plt.show()
                fig.canvas.draw()
    ...
    if not (plot == None):
        ...
        plt.title(
            "Decision function, iter=" + str(k) + ", acc=" + str(round(acc_test, 3))
        )
        plt.tight_layout()
        plt.show()
        fig.canvas.draw()
